...
def timing(f):
    ...
    def wrap(*args, **kw):
        ...
        if isinstance(result, pd.DataFrame) or isinstance(result, dask.dataframe.core.DataFrame):
            ...
            df_disp = ''
        logger.debug('[END] %r  %2.0f min' % (f.__name__,  round((te-ts)/60)) + memory_disp + df_mem + ' GB; ' + df_disp)
        if write_monitoring:
...
def create_or_load_result(f, *args, **kw):
    ...
    logger.debug('Signature after {}: {}'.format(f.__name__, GLOBAL_SIGNATURE.sig))
    file_path = '../data/Intermediate_Results\\' + GLOBAL_SIGNATURE.sig +str('.parquet.gzip')
    #file_path = '../data/Intermediate_Results\\' + f.__name__ +str('.parquet.gzip')
